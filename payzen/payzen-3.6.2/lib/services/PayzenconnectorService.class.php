...
/**
 * payzen_PayzenconnectorService
 * @package modules.payzen.lib.services
 */

class payzen_PayzenconnectorService extends payment_ConnectorService {
	/**
	 * @var payzen_PayzenconnectorService
	 */
	private static $instance;

	/**
	 * @return payzen_PayzenconnectorService
	 */
	...
	public function getNewDocumentInstance() {
		return $this->getNewDocumentInstanceByModelName('modules_payzen/payzenconnector');
	}

	/**
	 * Create a query based on 'modules_payzen/payzenconnector' model.
	 * Return document that are instance of modules_payzen/payzenconnector,
	 * including potential children.
	 * @return f_persistentdocument_criteria_Query
	 */
	public function createQuery() {
		return $this->pp->createQuery('modules_payzen/payzenconnector');
	}
	
	/**
	 * @param payzen_persistentdocument_payzenconnector $connector
	...
	public function setPaymentInfo($connector, $order) {
		...
		if ($transactionId != null) {
			// Paiement effectué : afficher les informations sur la transaction 
		...
		$ms->log("PayZen PAYMENT : prepare data to be sent to payment platform, clientIP: ".$_SERVER['REMOTE_ADDR']);

		// Use our custom class to generate the html
		$api = new PayzenApi('UTF-8');

		$language = strtolower($order->getPaymentLang()); // order language
		if(!in_array($language, $api->getSupportedLanguages())) {
			$language = $connector->getPayzen_language() ? $connector->getPayzen_language() : 'fr';
		} 

		// Get the currency to use
		$currency = $api->findCurrencyByAlphaCode($order->getPaymentCurrency());
		if(!currency) {
			// default PayZen currency
			$ms->log("PayZen PAYMENT : currency " . $order->getPaymentCurrency() . " not supported. Use PayZen default one, usedCurrency: EUR");

			$currency = $api->findCurrencyByAlphaCode('EUR');
		}
		
		$total = $currency->convertAmountToInteger($order->getPaymentAmount());
		
		$api->set('amount', 			$total);
		$api->set('order_id',			$order->getId());
		$api->set('contrib', 			'RBSChange3.6.2_1.0a');
		
		$api->set('currency', 			$currency->num);
		$api->set('language',			$language);
		
		$languages = $connector->getPayzen_available_languages();
		$langsArray = explode(',', $languages);
		if(in_array('', $langsArray)) {
			$api->set('available_languages',		'');
		} else {
			$api->set('available_languages',		implode(';', $langsArray));
		}
		
		$api->set('site_id',		    $connector->getPayzen_site_id());
		$api->set('key_test',		    $connector->getPayzen_key_test());
		$api->set('key_prod',			$connector->getPayzen_key_prod());
		$api->set('ctx_mode',			$connector->getPayzen_ctx_mode());
		$api->set('capture_delay', 		$connector->getPayzen_capture_delay());
		$api->set('validation_mode',	$connector->getPayzen_validation_mode());
		
		$paymentCards = $connector->getPayzen_payment_cards();
		$cardsArray = explode(',', $paymentCards);
		if(in_array('', $cardsArray)) {
			$api->set('payment_cards',		'');
		} else {
			$api->set('payment_cards',		implode(';', $cardsArray));
		}
		
		$api->set('cust_id',			strval($order->getPaymentUser()->getId()));
		
		$billAddress = $order->getPaymentBillingAddress();
		$api->set('cust_phone',   		$billAddress->getPhone());
		$api->set('cust_email',			$billAddress->getEmail());
		$api->set('cust_title',   		$billAddress->getTitle() ? $billAddress->getTitle()->getLabel() : '');
		$api->set('cust_first_name',	$billAddress->getFirstname());
		$api->set('cust_last_name',		$billAddress->getLastname());
		$api->set('cust_address',		trim($billAddress->getAddressLine1(). ' ' . $billAddress->getAddressLine2(). ' ' . $billAddress->getAddressLine3()));
		$api->set('cust_zip',			$billAddress->getZipCode());
		$api->set('cust_city',			$billAddress->getCity());
		$api->set('cust_state',			$billAddress->getProvince());
		$api->set('cust_country',		$billAddress->getCountry() ? strtoupper($billAddress->getCountry()->getCode()) : '');
		
		$shipAddress = $order->getPaymentShippingAddress();
		$api->set('ship_to_first_name', $shipAddress->getFirstname());
		$api->set('ship_to_last_name', 	$shipAddress->getLastname());
		$api->set('ship_to_street', 	$shipAddress->getAddressLine1());
		$api->set('ship_to_street2', 	trim($shipAddress->getAddressLine2() . ' ' . $billAddress->getAddressLine3()));
		$api->set('ship_to_city',  		$shipAddress->getCity());
		$api->set('ship_to_state',  	$shipAddress->getProvince());
		$api->set('ship_to_country', 	$shipAddress->getCountry() ? strtoupper($shipAddress->getCountry()->getCode()) : '');
		$api->set('ship_to_zip', 		$shipAddress->getZipCode());
		$api->set('ship_to_phone_num', 	$shipAddress->getPhone());
		
		$api->set('url_return',			$this->getReturnURL($connector));
		$api->set('return_mode',		$connector->getPayzen_return_mode());
		$api->set('redirect_enabled',	$connector->getPayzen_redirect_enabled());
		$api->set('redirect_success_timeout', 	$connector->getPayzen_redirect_success_timeout());
		$api->set('redirect_success_message', 	$connector->getPayzen_redirect_success_message());
		$api->set('redirect_error_timeout', 	$connector->getPayzen_redirect_error_timeout());
		$api->set('redirect_error_message', 	$connector->getPayzen_redirect_error_message());
		
		// Finaliser la commande : il faut afficher un formulaire de redirection vers la plateforme
 		$messageArray= array();
 		$messageArray[] ='<form name="payzen" method="POST" action="'. $connector->getPayzen_platform_url().'">';
 		$messageArray[] = $api->getRequestFieldsHtml();
 		
 		$submitText = LocaleService::getInstance()->transFO('m.payzen.fo.payment-action', array('attr'));
 		$messageArray[] = '	<input type="submit" class="button" value="' . $submitText . '" />';
 		$messageArray[] ='</form>';
 		
 		$ms->log("PayZen PAYMENT : client sent to payment platform with data : " . print_r($api->getRequestFieldsArray(), true));
 		
 		$connector->setHTMLPayment(implode("", $messageArray));
	}
	
	/**
	 * Overridden only for backward compatibility
	 * @param payment_persistentdocument_freeconnector $connector
	 * @param payment_Order $order
	 */
	protected function setPaymentStatus($connector, $order) {
		if(method_exists('payment_ConnectorService', 'setPaymentStatus')){
			parent::setPaymentStatus($connector, $order);
		} else {
			// in 3.5.x versions
	...
	/**
	 * Process payment result (save order status, send mail, ...)
	 * @param PayzenResponse $payzenResponse 
	 */
	public function validatePayment($payzenResponse) {
		try {
			$ms = payment_ModuleService::getInstance();
			
			$this->getTransactionManager()->beginTransaction();
			
			$ms->log("PayZen PAYMENT : transaction saving for order " . $payzenResponse->get('order_id') . " BEGIN");
			
			$bankResponse = $this->getBankResponse($payzenResponse);
			...
			$ms->log("PayZen PAYMENT : transaction saving for order " . $payzenResponse->get('order_id') . " END");
			
			$this->getTransactionManager()->commit();
			
			return true;
		} catch (Exception $e) {
			$ms->log("PayZen PAYMENT : transaction saving for order " . $payzenResponse->get('order_id') . " FAILED, message: " . $e->getMessage());
	...
	/**
	 * Construct the payment transaction object according to payment result.
	 * 
	 * @param PayzenResponse $payzenResponse
	 * @throws Exception
	 * @return payment_Transaction
	 */
	public function getBankResponse($payzenResponse) {
		$response = new payment_Transaction();
 		$response->setRawBankResponse($payzenResponse->raw_response);
		
 		$order = DocumentHelper::getDocumentInstance($payzenResponse->get('order_id'));
 		if (!($order instanceof payment_Order)) {
 			throw new Exception("Invalid bill reference: " . $payzenResponse->get('order_id'));
 		}
 		...
 		$response->setTransactionId($payzenResponse->get('trans_id'));
 		
 		if ($payzenResponse->isAcceptedPayment()) {
 			$response->setAccepted();
 			$msgKey = 'accepted-transaction';
 		} else {
 			$response->setFailed();
 			$msgKey = 'failed-transaction';
 		}

 		$msg = LocaleService::getInstance()->transFO('m.payzen.fo.' . $msgKey);
 		
 		if (Framework::inDevelopmentMode()) {
 			$msg .= "\n" . $payzenResponse->getLogString();
 		}
 		
 		if(!$payzenResponse->get('hash') && $payzenResponse->isAcceptedPayment() && $connector->getPayzen_ctx_mode() == 'TEST') {
 			// Order confirmed by return url : check url did not work
 			$msg .= "\n" . LocaleService::getInstance()->transFO('m.payzen.fo.check-url-warn'); 
 		}
 		$response->setTransactionText($msg);
 		$response->setDate($payzenResponse->get('trans_date'));
		return $response;
	}
	
	/**
	 * Return null if $id is not a payzen_persistentdocument_payzenconnector
	 * @param integer $id
	 * @return payzen_persistentdocument_payzenconnector
	 */
	public function getConnectorById($id) {
		try {
			...
			if ($document instanceof payzen_persistentdocument_payzenconnector) {
				return $document;
			}
		} catch (Exception $e) {
	...
	/**
	 * @param payment_persistentdocument_payzenconnector $connector
	 * @return String
	 */
	private function getServer($connector = null) {
	 ...
	 * @return String
	 */
	private function getReturnURL($connector = null) {
		$protocol =  ($connector !== null && $connector->getUseHTTPS()) ? "https://" : "http://";
		
		return $protocol . $this->getServer($connector) . "/payment/payzenReturn.php";
	}
